...
The LOOCV test error rate generated from the `cv.glm` function is approximately 0.25 (rounded up to 2 decimal places) or 25%. This means that on average, the model predicts results incorrectly about 25% of the time. This error rate is much lower than that generated from the manual implementation of LOOCV. This indicates that LOOCV estimates are highly variable. This could be due to the fact that the estimates for the MSE are based on only one observation from the dataset. 

```{r}
# Set K = 1089 to tell cv.glm to run LOOCV 
glm.fit1 <- glm(Direction ~ Lag1 + Lag2, data = Weekly, family = binomial)
cv.error1 = cv.glm(Weekly, glm.fit1, K = 1089)
cv.error1$delta
```
...
Using the `trainControl()` function from the `caret` package, I found that the LOOCV test error rate was (1 - 0.55) = 0.45 or 45%, which is comparable to the test error rate obtained in the manual implementation in (e). 

The test error rates for 10-Fold Cross Validation and Bootstrap resampling were also about 45%. 
